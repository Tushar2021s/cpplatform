...
export default function Problems() {
    ...
    const fetchProblems = async () => {
        ...
        try {
            ...
            setProblems(res.data.slice(0, 100))
        } catch (e) {
            console.error(e)
        } finally {
            setLoading(false)
    ...
    const fetchSolved = async () => {
        try {
            ...
            const ids = new Set<number>(res.data.map((s: any) => s.problem.id))
            setSolvedIds(ids)
        } catch (e) {}
    }
    ...
    // rating colors stay as functional difficulty indicators — this is intentional,
    // not decorative color. CP platforms universally use rating-tier colors.
                  ...
                  {solvedIds.has(problem.id) ? '✅' : '⬜'}
                                ...
                                <div className="min-w-0">
                                    <a href={problem.url}
                                       ...
                                       className="font-medium hover:underline transition truncate block">
                                        {problem.contestId}{problem.index}. {problem.name}
                                    </a>
                                    <div className="flex gap-1.5 mt-1.5 flex-wrap">
                                        {problem.tags.slice(0, 3).map(tag => (
                                            <span key={tag}
                                                  className="text-xs bg-white/[0.06] text-gray-400
